...
- **任务栏滚轮调音量（v2.3 新增）**：右下角开关一键启用 — 鼠标移到底部任务栏上滚动滚轮即可调节系统主音量，可设置每格幅度（默认 2%），中键单击切换静音。幅度=2% 时注入系统原生音量键（带系统自带 OSD）；其它幅度走 COM 精确控制。多显示器自动覆盖每个任务栏。
...
4. 勾选【开机自动启动】后开机自动运行
5. 需要任务栏滚轮调音量？主界面右下角勾选【任务栏滚轮调音量】即可
...
```
KeyMapper.cs        单文件源码（约 2800 行，含全部功能）
build.bat           本地一键编译脚本
...
- **v2.3.2**：修复任务栏滚轮调音量间歇性/启动即失效。滚动事件改为异步处理（钩子回调只做轻量命中判断，不再阻塞，避免触发系统低级钩子超时静默卸载）；启动时立即刷新任务栏位置缓存（消除启动空窗期）；30 秒自动重装钩子兜底；修复任务栏矩形缓存无限累积
- **v2.3.1**：修复切换声音输出设备后音量调节失效（音量调节改为自动识别当前激活设备，每次重新获取默认音频端点，无需重启）
- **v2.3**：新增"任务栏滚轮调音量"——主界面右下角开关，鼠标在任务栏上滚轮调音量（可设幅度），中键切换静音；多显示器支持；幅度 2% 时使用系统原生音量键（带系统自带 OSD）
